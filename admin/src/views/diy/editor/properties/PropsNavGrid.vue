<template>
  <div class="config-panel">
    <div class="config-row">
      ...
      <div class="config-control"><el-input-number v-model="form.columns" :min="2" :max="6" :controls="false" @change="emitUpdate" /></div>
    </div>
    <div class="config-row">
    ...
    <div class="config-section">导航项目</div>
    <div v-for="(item, i) in localItems" :key="i" class="config-card" draggable="true" @dragstart="onDragStart(i, $event)" @dragover.prevent @drop="onDrop(i)" @dragend="dragIdx = null">
      ...
      <div class="config-card__body">
        <el-input v-model="item.title" placeholder="标题" @change="emitUpdate" />
        <LinkPicker v-model="item.url" @update:model-value="emitUpdate" />
      </div>
    </div>
    <el-button type="primary" class="config-add-btn" @click="localItems.push({icon:'',title:'',url:''}); emitUpdate()"><i class="i-lucide:plus mr-1" />添加导航</el-button>
  </div>
</template>
